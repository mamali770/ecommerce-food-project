    ...
    <form action="{{ route('product.update', ['product' => $product->id]) }}" enctype="multipart/form-data" method="POST" class="row gy-4 mb-5">
        ...
        <div class="col-md-3">
            ...
            <select name="status" class="form-control">
                <option value="1" {{ $product->status ? 'selected' : '' }}>فعال</option>
                <option value="0" {{ $product->status ? '' : 'selected' }}>غیر فعال</option>
            </select>
            <div class="form-text text-danger">
                @error('status')
                    {{ $message }}
        ...
        <div class="col-md-3">
            ...
            <input name="sale_price" value="{{ $product->sale_price }}" type="text" class="form-control" />
            <div class="form-text text-danger">
                @error('sale_price')
                    {{ $message }}
                @enderror
        ...
        <div class="col-md-3">
            ...
            <input data-jdp name="date_on_sale_from" value="{{ $product->date_on_sale_from != null ? verta($product->date_on_sale_from)->format('Y/m/d H:i:s') : '' }}" type="text"
                class="form-control" />
            <div class="form-text text-danger">
        ...
        <div class="col-md-3">
            ...
            <input data-jdp name="date_on_sale_to" value="{{ $product->date_on_sale_to != null ? verta($product->date_on_sale_to)->format('Y/m/d H:i:s') : '' }}" type="text"
                class="form-control" />
            <div class="form-text text-danger">
